...
bool BehaviourSystem::addEntityCollisionSelector(const std::string& id,const std::string& selector){for(auto& d:entities_)if(d.id==id){d.mustCollideWith.push_back(selector);return true;}return false;}
