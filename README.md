...
Static-render policy is route-specific. The normal route keeps static ray tracing on the single CPU thread, the pthread route keeps it on the multithreaded CPU renderer, and the adaptive route alone may select the WebGL2 static tracer before falling back to MT CPU and then ST CPU. WebGL2 may still be used purely to present the already-rendered framebuffer on any route; that display step is independent of the static renderer.
